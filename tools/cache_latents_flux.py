...
from library import flux_train_utils, strategy_flux, flux_utils
...
def cache_to_disk(args: argparse.Namespace) -> None:
    ...
    model_type = "schnell" if args.is_schnell else "flux"

    # tokenizerを準備する：datasetを動かすために必要
    t5xxl_max_length = 256 if args.is_schnell else 512
    tokenize_strategy = strategy_flux.FluxTokenizeStrategy(t5xxl_max_length)
    tokenizer1, tokenizer2 = [tokenize_strategy.clip_l, tokenize_strategy.t5xxl]
    ...
    latent_strategy = strategy_flux.FluxLatentsCachingStrategy(
        args.cache_latents_to_disk,
        args.vae_batch_size,
        args.skip_latents_validity_check

    )
    vae = flux_utils.load_ae(model_type, args.ae, weight_dtype, "cpu")
        ...
        for i in range(0, b_size, vae_batch_size):
            ...
            for i, (image, absolute_path, resized_size) in enumerate(zip(images, absolute_paths, resized_sizes)):
                ...
                if args.skip_existing:
                    if latent_strategy.is_disk_cached_latents_expected(
            ...
            if len(image_infos) > 0:
                latent_strategy.cache_batch_latents(vae, image_infos, flip_aug, alpha_mask, random_crop)
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--is_schnell",
        action="store_true",
        help="use schnell model / schnell モデルを使う",
    )
    parser.add_argument(
        "--no_half_vae",
        action="store_true",
